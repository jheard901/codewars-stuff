	...
	while (upDownTraversed != UP_DOWN_EXPECTED || leftRightTraversed != LEFT_RIGHT_EXPECTED)
	{
		...
		if (bDirection)
		{
			...
			else
			{
				...
				std::vector<int>::const_iterator it = snail_map[MATRIX_SIZE - 1 - rightLeftCount].end() - 2 - rightLeftCount;

				for (int i = 0 + rightLeftCount; i < MATRIX_SIZE - leftRightCount; i++)
				{
					result.push_back(*it);
					if (it != snail_map[MATRIX_SIZE - 1 - rightLeftCount].begin()) { it--; }
					bTraversed = true;
				}
